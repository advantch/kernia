...
async def test_full_passkey_register_and_authenticate() -> None:
    ...
    attestation = authenticator.register(challenge=challenge, origin=ORIGIN, rp_id=RP_ID)
    r = await driver.request(
...
async def test_register_rejects_tampered_challenge() -> None:
    ...
    fake_attestation = authenticator.register(challenge=b"\x00" * 32, origin=ORIGIN, rp_id=RP_ID)
    r = await driver.request(
