...
Tests should assert that admin-only endpoints reject normal users, dashboard counters are database-backed, secret fields are redacted, and audit events are written for admin actions. The demo should show real unavailable states for optional SSO, SCIM, and Stripe setup instead of rendering fake success.
