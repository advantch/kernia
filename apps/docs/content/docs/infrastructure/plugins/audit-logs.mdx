...
Tests should prove that config writes, billing syncs, API key changes, SSO changes, SCIM token changes, and session revocations create audit events with redacted metadata. Browser tests should show audit entries after admin actions in the SaaS demo.
