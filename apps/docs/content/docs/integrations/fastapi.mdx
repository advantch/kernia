...
A real FastAPI + Vite demo should cover email sign-up, email sign-in, session refresh, logout, OAuth provider unavailable states, profile update, linked accounts, sessions revoke, API keys, and admin configuration. Tests should drive HTTP requests against the mounted FastAPI app rather than calling route handlers directly.
