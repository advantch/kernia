...
Unit tests can register a sink handler before triggering the action under
test:
...
get_bus(auth.context).on("organization.member.added", capture)

# ...drive the route that should emit...

assert len(seen) == 1
assert seen[0].user_id == new_member_id
~~~
