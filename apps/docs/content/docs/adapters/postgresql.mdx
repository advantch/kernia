...
Run the FastAPI demo and adapter tests against PostgreSQL before treating a plugin as production-ready. Cover sign-up, sign-in, OAuth callback, session refresh, logout, API key usage, organization membership, Stripe webhooks, and admin config writes.
