...
cd apps/docs && pnpm docs:quality && pnpm typecheck && pnpm build
```

If a test cannot run locally because it needs a provider credential, add a mock provider test and document the live-test environment variables.
