"""`better-auth generate` — emit an Alembic migration for the resolved schema.
...
import click
from better_auth.db.migrations.codegen import emit_migration

from better_auth_cli.utils import (
    collect_models,
...
@click.option(
    ...
    default=None,
    help="Output file. Defaults to alembic/versions/<rev>_better_auth_schema.py.",
)
@click.option(
    "--cwd",
...
def generate(config_path: str, output: str | None, cwd: str, force: bool) -> None:
    ...
    source = emit_migration(models, revision=revision, message="better-auth schema")

    if output is None:
        out_dir = Path(cwd) / "alembic" / "versions"
        out_dir.mkdir(parents=True, exist_ok=True)
        out_path = out_dir / f"{revision}_better_auth_schema.py"
    else:
