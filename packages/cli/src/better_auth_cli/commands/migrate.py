"""`better-auth migrate` — run `alembic upgrade head` against the user's DB.
...
`options.database` adapter or from the `BETTER_AUTH_DATABASE_URL` env var if
present.
"""
...
def _resolve_db_url(auth, override: str | None) -> str:
    if override:
        return override
    env_url = os.environ.get("BETTER_AUTH_DATABASE_URL") or os.environ.get("DATABASE_URL")
    if env_url:
        return env_url
    ...
    raise click.ClickException(
        "No database URL could be resolved. Pass --db-url or set BETTER_AUTH_DATABASE_URL."
    )
...
def migrate(config_path: str, cwd: str, db_url: str | None, revision: str) -> None:
    ...
    if not versions_dir.exists() or not any(versions_dir.glob("*.py")):
        raise click.ClickException(
            ...
            f"Run `better-auth generate` first."
        )
...
_MINIMAL_ENV_PY = '''"""Auto-generated minimal Alembic env for better-auth."""
