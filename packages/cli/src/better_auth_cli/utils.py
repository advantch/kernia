...
def load_config_module(config_path: str | Path) -> ModuleType:
    ...
    mod_name = f"_better_auth_user_config_{hashlib.sha1(str(path).encode()).hexdigest()[:8]}"
...
def find_auth_handle(mod: ModuleType) -> BetterAuth:
    """Return the `BetterAuth` instance from a user config module.

    Convention: the module exposes a top-level `auth` object. Falls back to the
    first `BetterAuth` instance found in the module namespace.
    """
    candidate = getattr(mod, "auth", None)
    if isinstance(candidate, BetterAuth):
        return candidate
    for value in vars(mod).values():
        if isinstance(value, BetterAuth):
            return value
    raise click.ClickException(
        "Config module did not expose a `BetterAuth` instance (e.g. `auth = init(...)`)."
    )
...
def collect_models(auth: BetterAuth) -> tuple[Any, ...]:
