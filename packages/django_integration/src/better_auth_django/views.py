"""Django view that bridges to the better-auth ASGI router.
...
boundary. Every better-auth request therefore costs one thread hop. This is a
...
def django_request_to_scope(
    ...
    *,
    ...
    # fields better-auth actually reads (path, method, headers, query_string,
...
class BetterAuthView(View):
    """Class-based view that funnels every method onto the auth router.

    Usage::

        BetterAuthView.as_view(auth=my_auth, base_path="/api/auth")
    """
    ...
    def dispatch(self, request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
        if self.auth is None:
            raise RuntimeError(
                "BetterAuthView requires an `auth` kwarg passed via .as_view()"
            )
...
def setup(
    ...
    prefix lands on ``BetterAuthView``.
    """
    prefix = url_prefix.strip("/")
    return [
        path(
            f"{prefix}/<path:rest>",
            BetterAuthView.as_view(auth=auth, base_path="/" + prefix),
        ),
        path(
            f"{prefix}",
            BetterAuthView.as_view(auth=auth, base_path="/" + prefix),
