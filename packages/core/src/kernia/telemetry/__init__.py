...
signal — adding it to `KerniaOptions.plugins` enables emission. When opted in,
the plugin emits a single startup event with: kernia version,
registered plugin ids, adapter kind. No PII.

`KerniaOptions.advanced["telemetry"] = False` force-suppresses emission even
