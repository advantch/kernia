...
``kernia_oauth_provider`` package. This module remains importable so legacy
call sites keep working, but every entry point emits a :class:`DeprecationWarning`
and delegates to ``kernia_oauth_provider``.
...
Migration::
    ...
    # after
    from kernia_oauth_provider import oauth_provider, OAuthProviderOptions
...
_DEPRECATION = (
    "kernia.plugins.oidc_provider is deprecated; install and use the "
    "`kernia_oauth_provider` package instead "
    "(from kernia_oauth_provider import oauth_provider, OAuthProviderOptions)."
)
...
def _require_oauth_provider() -> Any:
    try:
        import kernia_oauth_provider as mod
    except ImportError as exc:  # pragma: no cover - depends on install layout
        raise ImportError(
            "The oidc_provider shim delegates to the `kernia_oauth_provider` "
...
def oidc_provider(options: Any) -> KerniaPlugin:
    """Deprecated. Delegates to ``kernia_oauth_provider.oauth_provider``.
...
def __getattr__(name: str) -> Any:
    ...
    Lets ``from kernia.plugins.oidc_provider import OAuthProviderOptions``
