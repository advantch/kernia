"""FastMCP authentication backed by a kernia issuer.

This is the Python analogue of Better Auth's ``withMcpAuth`` (the upstream
TypeScript project's ``packages/better-auth/src/plugins/mcp``): it lets a
`FastMCP` server accept the OAuth access tokens minted by kernia's ``mcp`` /
``oauth_provider`` plugins, validate them against the issuer's JWKS, and gate
tools by scope.
...
clients at the kernia authorization server.
"""
...
class BetterAuthTokenVerifier(TokenVerifier):
    """Validates kernia-issued MCP access tokens against the local JWKS.
...
def mcp_auth(
    ...
    **kwargs: Any,
    ...
    """Build a FastMCP auth provider that accepts kernia MCP tokens.

    ``base_url`` is this MCP resource server's public URL. ``authorization_servers``
    defaults to the kernia issuer derived from the ``mcp`` plugin options, so
