...
| oauth_provider | 147 | 278 | 53 % — JWT + opaque-token models + pairwise PPID subjects + configurable per-endpoint rate limits + refresh_token client-auth (Basic/body, mismatch, disabled-client → invalid_client) + OIDC RP-initiated logout (end-session, `sid` claim, admin-only `enable_end_session`, post-logout redirect) + DCR validation (empty-body → 400, `skip_consent` rejected as privileged); remaining DB-token-table + unauthenticated-DCR-override cases unported |
